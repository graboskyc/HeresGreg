...
<html lang="en">
  ...
  <body>
    ...
    <div class="container">
      ...
      <?php
        $conn = connectDB();
        $sql = "SELECT DATE_FORMAT(created, '%Y') as 'year', DATE_FORMAT(created, '%m') as 'month', COUNT(id) as 'total', QUARTER(created) as quarter FROM media where isFavorite = 1 GROUP BY year(created), quarter(created)";
        ...
        foreach($result as $r)
        {
            ...
            $retStr = $retStr . '<li>
              ...
              &nbsp;'.$r['total'].'</i></span><a href="vidList.php?view='.$r['quarter'].'_'.$r['year'].'"> in <b>Q'.$r['quarter'].' '.$r['year'].'</b>
              </a></li>';
